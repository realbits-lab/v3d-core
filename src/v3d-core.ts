...
export class V3DCore implements GLTFLoaderExtensionObserver {
   ...
   * Shadow generators
   */
  private _shadowGenerators: Map<IShadowLight, ShadowGenerator> = new Map<
    IShadowLight,
    ShadowGenerator
  >();
  ...
  private _afterRenderFunc: (eventData: Scene, eventState: EventState) => void =
    () => {
  ...
  public addVRMManager(manager: VRMManager) {
    if (manager) this.loadedVRMManagers.push(manager);
  }
  ...
  public constructor(
    ...
    if (!this.scene) this.scene = new Scene(this.engine);
    else this.engine = this.scene.getEngine();
   ...
   */
  public createSkyBox(size: number, textureName?: string) {
    if (!this.skyBox) {
      this.skyBox = new v3DSkyBox(
        this.scene,
        textureName ? textureName : "texture/skybox",
        size
      );
    }
  }
  ...
  public enableShabows(light?: IShadowLight) {
    if (light) {
      if (!this._shadowGenerators.has(light)) {
        ...
        console.warn(
          "Light " + light.name + " already has a shadow generator!"
        );
      }
    } else {
  ...
  public createAnimation(
    ...
    if (keyFrames.length < 1) throw Error("Key Frames empty");
    ...
    if (easingFunction) {
      if (easingMode) easingFunction.setEasingMode(easingMode);
  ...
  private setupObservable() {
    ...
    // Camera
    this.scene.onBeforeRenderObservable.add(() => {
      for (const f of this._cameraOnBeforeRenderFunc) f();
    });
